...
class BookingManager:
    ...
    async def process_user_message(
        self, phone: str, message_text: str
    ) -> Tuple[str, bool]:
        """
        ...
        user_message = MessageCreate(
            content=message_text, sender_id=phone, is_from_bot=False
        )
    ...
    async def _handle_collecting_info(
        ...
        if booking_data:
            try:
                ...
                confirmation_message = await self._create_confirmation_message(
                    conversation.id, booking_data
                )

                # Update conversation state
                await ConversationRepository.update(
                    conversation.id, {"state": ConversationState.CONFIRMING}
                )

                # Save the confirmation message
                bot_message = MessageCreate(
                    content=confirmation_message, sender_id="bot", is_from_bot=True
                )
                await MessageRepository.create(conversation.id, bot_message)
    ...
    async def _handle_confirming(
        ...
        if is_confirmed and not is_rejected:
            ...
            booking_data = await self._extract_booking_data_from_conversation(
            ...
            if booking_data:
                try:
                    ...
                    completion_message = (
                        "Спасибо за подтверждение! Ваша заявка на запись принята. "
                    ...
                    bot_message = MessageCreate(
                        content=completion_message, sender_id="bot", is_from_bot=True
                    )
                    await MessageRepository.create(conversation.id, bot_message)
                ...
                await ConversationRepository.update(
                    conversation.id, {"state": ConversationState.GREETING}
                )
            ...
            # Add bot message to conversation
            bot_message = MessageCreate(
                content=correction_message, sender_id="bot", is_from_bot=True
            )
            await MessageRepository.create(conversation.id, bot_message)
            ...
            # Add bot message to conversation
            bot_message = MessageCreate(
                content=clarification_message, sender_id="bot", is_from_bot=True
            )
            await MessageRepository.create(conversation.id, bot_message)
    ...
    async def _handle_completed(
        ...
        if any(phrase in message_lower for phrase in new_booking_phrases):
            ...
            # Add bot message to conversation
            bot_message = MessageCreate(
                content=thank_you_message, sender_id="bot", is_from_bot=True
            )
            await MessageRepository.create(conversation.id, bot_message)

            return thank_you_message

    async def _create_confirmation_message(
        self, conversation_id: UUID, booking: BookingFunctionArgs
    ) -> str:
        """
        ...
        if booking.preferred_contact_time:
            ...
            contact_time_str = time_map.get(
                booking.preferred_contact_time, "Не указано"
            )
        ...
        return (
            f"Спасибо за предоставленную информацию! Пожалуйста, проверьте детали вашей записи:\n\n"
    ...
    async def _extract_booking_data_from_conversation(
        self, conversation_id: UUID, messages: List[Message]
    ) -> Optional[BookingFunctionArgs]:
        """
        ...
        for msg in reversed(messages):
            if msg.is_from_bot and msg.content.startswith(
